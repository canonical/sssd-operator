...
def test_sssd_ops_error() -> None:
    ...
    with pytest.raises(sssd.SSSDOpsError) as exc_info:
        raise sssd.SSSDOpsError(message)
    assert exc_info.value.message == message


def test_install(mocker: MockerFixture, mock_run, fs: FakeFilesystem) -> None:
    """Test `sssd.install()` function."""
    add_package = mocker.patch.object(apt, "add_package")
    ...
    assert fs.exists(sssd.SSSD_CONFIG_FILE)
...
def test_remove(mocker: MockerFixture, fs: FakeFilesystem) -> None:
    """Test `sssd.remove()` function."""
    remove_package = mocker.patch.object(apt, "remove_package")
...
def test_version(mock_run) -> None:
    ...
    assert mock_run.call_args[0][0] == ["dpkg", "-l", "sssd"]
...
def test_restart(mock_run) -> None:
    """Test `sssd.restart()` function."""
    sssd.restart()
    assert mock_run.call_args[0][0] == ["systemctl", "restart", "sssd"]


def test_enable(mock_run) -> None:
    """Test `sssd.enable()` function."""
    sssd.enable()
    assert mock_run.call_args[0][0] == ["systemctl", "enable", "--now", "sssd"]


def test_disable(mock_run) -> None:
    """Test `sssd.disable()` function."""
    sssd.disable()
    assert mock_run.call_args[0][0] == ["systemctl", "disable", "--now", "sssd"]


def test_read(fs: FakeFilesystem) -> None:
...
def test_edit(fs: FakeFilesystem) -> None:
...
def test_domains(fs: FakeFilesystem) -> None:
    """Test `sssd.domains()` function."""
    fs.create_file("/etc/sssd/sssd.conf", contents=MOCK_FULL_SSSD_CONFIG)
    assert sssd.domains() == ["ldap"]


def test_add_ldap_domain(mocker: MockerFixture, fs: FakeFilesystem) -> None:
...
def test_update_ldap_domain(mocker: MockerFixture, fs: FakeFilesystem) -> None:
...
def test_remove_ldap_domain(mocker: MockerFixture, fs: FakeFilesystem) -> None:
...
def test_add_tls_certs(mock_run, fs: FakeFilesystem) -> None:
...
def test_remove_tls_certs(mock_run, fs: FakeFilesystem) -> None:
